	...
	if ((isset($_GET['article']) && $_GET['article']!="") || (isset($_GET['a']) && $_GET['a']!=""))
	{
		if (isset($_GET['a']) && $_GET['a']!="")
		{
			...
			FROM
				articles
			WHERE
				alias='".mysql_real_escape_string($_GET['a'])."'
			ORDER BY 
				rev DESC
			LIMIT 1
			;";
		...
		if (LOGIN)
		{
			...
			if (mysql_num_rows($res)>0)
			{
				...
				if (isset($_POST['submit']))
				{
					...
					{
						$hres = dbquery("SELECT MAX(rev) FROM articles WHERE hash='".$arr['hash']."';");
						$harr = mysql_Fetch_row($hres);
						if (dbquery("INSERT INTO 
						articles 
						(
							hash,
							alias,
						...
						);"))
						{
							...
							//dbquery("DELETE FROM help_tag_rel WHERE domain='wiki' AND item_id=".$arr['id'].";");
							foreach ($tags as $t)
							{
								$t = trim($t);
								if ($t != "")
								{
									...
									$sql="INSERT INTO help_tag_rel (domain,tag_id,item_id) VALUES ('wiki',".$tagId.",".$id.");";
									dbquery($sql);
								}
							}						
