...
$site_title = 'Häufig gestellte Fragen (FAQ)';
...
$submitInfoText = '<b>Wichtig:</b> Nur eine Frage auf einmal stellen! Beschreibe den Sachverhalt möglichst präzise, versuche dabei aber den Text kurz zu halten. Bitte keine Fehlerberichte wie \'Login geht nicht...\',
    diese bitte ins <a href="' . ForumBridge::url('board', get_config('support_board')) . '">Support-Forum</a> stellen.';
    ...
    if (LOGIN) {
        ...
        if ($arr = mysql_fetch_row($res)) {
            ...
            if ($arr = mysql_fetch_row($res)) {
                if ($_GET['vote'] == "up" && $arr[0] == 1 || $_GET['vote'] == "down" && $arr[0] == -1) {
                    dbquery("
                    DELETE FROM " . dbtable('faq_comment_vote') . "
                    WHERE
                        comment_id=" . $_GET['voteanswer'] . "
                        AND user_id=" . $_SESSION['etoahelp']['uid'] . "
                    ;");
                    dbquery("
                    UPDATE " . dbtable('faq_comments') . "
                    SET comment_vote=comment_vote+(" . ($_GET['vote'] == "up" ? -1 : 1) . ")
                    WHERE comment_id=" . $_GET['voteanswer'] . ";");
                } else {
                    dbquery("
                    UPDATE " . dbtable('faq_comment_vote') . "
                    SET
                        value=" . (-$arr[0]) . "
                    WHERE
                        comment_id=" . $_GET['voteanswer'] . "
                ...
                $v = $_GET['vote'] == "up" ? 1 : -1;
                dbquery("
                INSERT INTO " . dbtable('faq_comment_vote') . "
...
elseif (isset($_GET['faq']) && $_GET['faq'] > 0) {
    ...
    if (mysql_num_rows($res) > 0) {
        ...
        echo "<h2 class=\"faqquestiontitle\">" . StringUtil::text2html($arr['faq_question']) . "</h2>";
        echo popText();
        ...
        if ($nr > 0) {
            ...
            while ($carr = mysql_fetch_array($cres)) {
                ...
                $timestr = $carr['comment_time'] > 0 ? ", " . StringUtil::diffFromNow($carr['comment_time']) : ", vor langer Zeit";
        ...
        $tres = dbquery("SELECT t.id, t.name
        ...
        foreach ($tags as $k => $v) {
            $tres = dbquery("
                SELECT i.faq_id
                FROM " . dbtable('help_tag_rel') . " r
                INNER JOIN " . dbtable('faq') . " i
        ...
        if (count($faqs) > 0) {
            ...
            foreach ($faqs as $k => $v) {
                ...
                if (mysql_num_rows($sres) > 0) {
                    ...
                    " . StringUtil::text2html($sarr['faq_question']) . "</a></li>";
                }
                if (++$i == 5)
                    break;
            }
    ...
    /*echo "<br/>";
    <input type=\"button\" onclick=\"document.location='?page=$page'\" value=\"Zurück zu den Fragen\" /> &nbsp; */
}
...
* Suche *
********/
elseif ((isset($_POST['faq_search']) && $_POST['faq_search'] != "" && ($_POST['faq_search_keyword'] != "" || $_POST['faq_search_fulltext'] != "")) || (isset($_GET['s']) && $_GET['s'] != "")) {
    //echo "<h2>Suche</h2>";
    if ($_GET['s'] != "") {
        $s = $_GET['s'];
        $_POST['faq_search'] = $s;
    ...
    $res = dbquery($sql);
    if (mysql_num_rows($res) > 0) {
        echo '<p>Suchergebnisse für <b>' . $_POST['faq_search'] . '</b>:</p>';
        echo "<ul>";
        ...
        if (LOGIN && $arr['faq_user_id'] == $_SESSION['etoahelp']['uid']) {
            if ($_POST[encfname('faq_user_question_submit')] != "") {
                if ($_POST[encfname('faq_user_question')] != "" && $_POST[encfname('faq_description')] != "") {
                    ...
                    dbquery("
                        DELETE FROM " . dbtable('help_tag_rel') . "
                        WHERE domain='faq' AND item_id=" . $id . ";");
                    foreach ($tags as $t) {
                        $t = trim($t);
                        if ($t != "") {
                            $res = dbquery("
    ...
    if ($_POST[encfname('faq_user_question_submit')] != "") {
        if ($_POST[encfname('faq_user_question')] != "" && $_POST[encfname('faq_description')] != "") {
            ...
            if (mysql_num_rows($res) == 0) {
                ...
                mail(get_config('faq_admin'), "EtoA-FAQ: Neue Frage", $text);
            } else {
...
Die Frage wird anschliessend durch einen Admin beantwortet und freigeschaltet.</p>';
    ...
    if (!LOGIN) {
        ...
        <p><label>E-Mail (wird nicht veröffentlicht):</label><br/>
...
***********/
    ...
    if ($cat == "my" && !LOGIN) {
        ...
        if ($nr > 0) {
            ...
            while ($arr = mysql_fetch_array($res)) {
                $text = StringUtil::text2html($arr['faq_question']);
                if (stristr($text, "<br"))
                ...
                if (isset($view) && $view == "short") {
                    ...
                    if ($arr['faq_updated'] > $arr['faq_user_time'])
                        $timestr .= ", " . StringUtil::diffFromNow($arr['faq_updated']) . " aktualisiert";
